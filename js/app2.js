...
function getMorePins(){
    ...
    prevURL = window.location.pathname;
                clearURL();
                fetchMore = false;
                PDK.request('/v1/boards/' + $('#username').text() + '/' + $('.currentBoard').first().attr('data-urlBoard') + '/pins', {fields: 'image,url,color,description,domain,link', limit : (next === 'first' ? 100 : 50), cursor : (next === 'first' ? '' : next)}, function(response){
